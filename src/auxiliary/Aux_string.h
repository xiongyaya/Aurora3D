...
namespace Aurora3D
{
	...
	//	static const Ch* ParsePattern(const Ch* src, const Ch* pattern, Args... args)
	//	{
...
#define STRING_PLACE_SEQUENCE "(?)"
	...
	public:
		...
		Container Format(const CharType *fmt, ...)
		{
		...
		template<>
		Container Format<0>(const CharType* fmt, ...)
		{
		...
		template<typename ...Args, size_type ArgsSize = sizeof...(Args)>
		const CharType* Parse(const AuxString& src, const AuxString& fmt, Args... args)
		{
