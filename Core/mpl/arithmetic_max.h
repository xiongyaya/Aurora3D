...
namespace Aurora3D
{
	namespace mpl
	{
		...
		template<typename T, T t1, T... others> struct Max :public Integral_<T, IntegralMax(t1, Max<T, others...>::value)> {};
		template<typename T, T t> struct Max<T, t> :public Integral_<T, t> {};
		...
		template<typename T> struct TypeMax<T> :public Integral_<typename T::value_type, T::value> {};
